...
def load_langpair_dataset(
    ...
    for k in itertools.count():
        ...
        srcbert_dataset = data_utils.load_indexed_dataset(
            bertprefix + src, dataset_impl=dataset_impl
        )
...
@dataclass
class TranslationBertConfig(FairseqDataclass):
    ...
    bert_model_name: str = field(
        default='bert-base-uncased',metadata={'help':'bert model name'}
    )
    encoder_ratio: float = field(
        default=1.,metadata={'help':'encoder_ratio'}
    )
    bert_ratio: float = field(
        default=1.,metadata={'help':'bert_ratio'}
    )
    finetune_bert: bool = field(
        default=True,metadata={'help':'finetune-bert'}
    )
    mask_cls_sep: bool = field(
        default=True,metadata={'help':'mask-cls-sep'}
    )
    warmup_from_nmt: bool = field(
        default=True,metadata={'help':'warmup-from-nmt'}
    )
    warmup_nmt_file: str = field(
        default='checkpoint_nmt.pt',metadata={'help':'checkpoint_nmt.pt'}
    )
    bert_gates: list = field(
        default_factory=lambda: [1, 1, 1, 1, 1, 1],metadata={'help':'bert_gates'}
    )
    bert_first: bool = field(
        default=False,metadata={'help':'bert_first'}
    )
    encoder_bert_dropout: bool = field(
        default=True,metadata={'help':'encoder_bert_dropout'}
    )
    ...
    encoder_bert_mixup: bool = field(
        default=True,metadata={'help':'encoder-bert-mixup'}
    )
    decoder_no_bert: bool = field(
        default=True,metadata={'help':'decoder_no_bert'}
    )
...
@register_task("translation_bert", dataclass=TranslationBertConfig)
class TranslationBertTask(FairseqTask):
    """
