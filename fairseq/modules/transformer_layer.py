...
class TransformerBertEncoderLayer(TransformerEncoderLayerBase):
    def __init__(
        self, cfg, bert_gate=True
    ):
        super().__init__(
            cfg
        )
        self.bert_attn = self.build_self_attention_bert(self.embed_dim, cfg)
        self.encoder_ratio = cfg.encoder_ratio
        self.bert_ratio = cfg.bert_ratio
        self.encoder_bert_dropout = cfg.encoder_bert_dropout
        self.encoder_bert_dropout_ratio = cfg.encoder_bert_dropout_ratio
        assert self.encoder_bert_dropout_ratio >= 0. and self.encoder_bert_dropout_ratio <= 0.5
        self.encoder_bert_mixup = cfg.encoder_bert_mixup
...
class TransformerBertDecoderLayer(TransformerDecoderLayerBase):
    def __init__(
    ...
    def forward(
        ...
        encoder_out: Optional[torch.Tensor] = None,
        encoder_padding_mask: Optional[torch.Tensor] = None,
        bert_encoder_out: Optional[torch.Tensor] = None,
        bert_encoder_padding_mask: Optional[torch.Tensor] = None,
        incremental_state: Optional[Dict[str, Dict[str, Optional[Tensor]]]] = None,
        prev_self_attn_state: Optional[List[torch.Tensor]] = None,
        prev_attn_state: Optional[List[torch.Tensor]] = None,
        self_attn_mask: Optional[torch.Tensor] = None,
