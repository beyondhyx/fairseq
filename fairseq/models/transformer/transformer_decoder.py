...
class TransformerBertDecoder(TransformerDecoderBase): 
    ...
    def build_decoder_layer(self, cfg, no_encoder_attn=False):
        if cfg.decoder_no_bert:
            ...
            layer = transformer_layer.TransformerBertDecoderLayer(cfg, no_encoder_attn, bert_gate=[x == 1 for x in cfg.bert_gates])
        checkpoint = cfg.checkpoint_activations
        if checkpoint:
            offload_to_cpu = cfg.offload_activations
            layer = checkpoint_wrapper(layer, offload_to_cpu=offload_to_cpu)
     ...
     def forward(
        ...
        prev_output_tokens,
        encoder_out: Optional[Dict[str, List[Tensor]]] = None,
        bert_encoder_out: Optional[Dict[str, List[Tensor]]] = None,
        incremental_state: Optional[Dict[str, Dict[str, Optional[Tensor]]]] = None,
        features_only: bool = False,
        full_context_alignment: bool = False,
        alignment_layer: Optional[int] = None,
    ...
    def extract_features(
        ...
        prev_output_tokens,
        encoder_out: Optional[Dict[str, List[Tensor]]],
        bert_encoder_out: Optional[Dict[str, List[Tensor]]],
        incremental_state: Optional[Dict[str, Dict[str, Optional[Tensor]]]] = None,
        full_context_alignment: bool = False,
        alignment_layer: Optional[int] = None,
        alignment_heads: Optional[int] = None,
    ...
    def extract_features_scriptable(
        ...
        for idx, layer in enumerate(self.layers):
            ...
            x, layer_attn, _ = layer(
                ...
                enc,
                padding_mask,
                bert_encoder_out['bert_encoder_out'][0],
                bert_encoder_out['bert_encoder_padding_mask'][0],
                incremental_state,
                self_attn_mask=self_attn_mask,
                self_attn_padding_mask=self_attn_padding_mask,
                need_attn=bool((idx == alignment_layer)),
